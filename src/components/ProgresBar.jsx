        ...
        {steps.map((step, index) => (
          <React.Fragment key={step.id}>
            <div 
              ...
              <div className={`absolute z-50 -left-12 size-14 shadow-md p-3 border border-4 ${
                (step.completed === true && activeStep > step.id) 
                ? 'bg-blue-800 '
                :activeStep === step.id
                ? 'bg-yellow-400'
                : 'bg-gray-300'} flex items-center justify-center rounded-full w-14 h-14`}> 
                <img 
                src={step.icon} 
                alt={step.label} 
                className={`size-14"${
              ...
              <div
                className={`flex items-center relative ${
                  (step.completed === true && activeStep > step.id)
                    ? 'bg-blue-800 text-white' // Fondo amarillo para paso activo
                    : activeStep==step.id   // Fondo gris para pasos inactivos
                    ? 'bg-yellow-400'
                    : 'bg-gray-300'
                }`}
